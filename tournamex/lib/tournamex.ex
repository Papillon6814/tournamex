defmodule Tournamex do
  ...
  defp generate(list) when length(list) >= 0 do
    ...
    |> Enum.shuffle()
    |> case do
      shuffled when length(shuffled) == 1 -> hd(shuffled)
      shuffled when length(shuffled) == 2 -> shuffled
      shuffled ->
        b = shuffled
  ...
  def initialize_match_list_with_fight_result(match_list, result \\ []) do
    Enum.reduce(match_list, result, fn match, acc ->
      case match do
        x when is_integer(match) -> acc ++ [%{"user_id" => x, "is_loser" => false}]
        x when is_list(match)    -> acc ++ [initialize_match_list_with_fight_result(x)]
        x                        -> x
      end
    end)
  end
  ...
  def initialize_match_list_of_team_with_fight_result(match_list, result \\ []) do
    Enum.reduce(match_list, result, fn match, acc ->
      case match do
        x when is_integer(match) -> acc ++ [%{"team_id" => x, "is_loser" => false}]
        x when is_list(match)    -> acc ++ [initialize_match_list_of_team_with_fight_result(x)]
        x                        -> x
      end
    end)
  end
  ...
  def renew_match_list_with_loser(match_list, loser) when is_integer(loser),
    do: renew_defeat(match_list, loser)
  ...
  def win_count_increment(match_list, user_id) when is_integer(user_id),
    do: renew_win_count(match_list, user_id)
  ...
  def check_lose?(match_list, user_id) when is_integer(user_id), do: check?(match_list, user_id)
  def check_lose?(_, _),                                         do: nil

  defp check?(match_list, user_id, result \\ false) do
    Enum.reduce(match_list, result, fn match, acc ->
      case match do
        %{"user_id" => uid, "is_loser" => is_loser} when uid == user_id -> is_loser
        x when is_map(x)                                                -> acc
        x when is_list(x)                                               -> check?(x, user_id, acc)
        _                                                               -> acc
      end
    end)
  end
  ...
  def put_value_on_brackets(match_list, key, value, result \\ []) when is_map(value) do
    Enum.reduce(match_list, result, fn match, acc ->
      case match do
        %{"user_id" => user_id} when user_id == key -> acc ++ [Map.merge(match, value)]
        %{"team_id" => team_id} when team_id == key -> acc ++ [Map.merge(match, value)]
        x when is_map(x)                            -> acc ++ [x]
        x when is_list(x)                           -> acc ++ [put_value_on_brackets(x, key, value)]
        x                                           -> acc ++ [x]
      end
    end)
  end
  ...
  def delete_loser([a, b], loser) when is_integer(a) and is_integer(b) do
    case [a, b] -- loser do
      list when length(list) == 1 -> hd(list)
      list                        -> list
    end
  end
